  ...
  <div id="watchingTravel">
    ...
    <div class="position leftBox">
      <div v-if="memory_type">
        <h3 style="margin-bottom: 10px">出行模式：</h3>
        <ul>
          <li
...
export default {
  ...
  mounted() {
    this.getDataset();
    let memory = localStorage.getItem("memory_travel");
    if (memory) {
      ...
      this.$http.get(path, params).then((res) => {
        if (res.result.code == 1) {
          ...
          for (let i = 0; i < res.result.data.length; i++) {
            ...
            localStorage.setItem("memory_travel", JSON.stringify(memoryData));
          }
        }
      });
    },
